...
const BackgroundImage: FC<IBackgroundImage> = ({ token, rest, t }) => {
  ...
  React.useEffect(() => {
    restApi
      ...
      .then((res: TRest) => {
        setRestData(res)
        if (res.colorsSchema_id.useBackgroundImg !== null) {
          if (res.colorsSchema_id.useBackgroundImg.length > 0) {
            setFileList([
              {
                url: `${BASE_URL_CDN}/${res.colorsSchema_id.useBackgroundImg}`,
                name: res.colorsSchema_id.useBackgroundImg,
                uid: '0'
              }
            ])
            setLoading(true)
  ...
  const onFinish = (values: any) => {
    ...
    if (isChangeImageSale) {
      if (values.image === null) {
        colorApi
          .updateColor(
            { ...restData.colorsSchema_id, useBackgroundImg: null },
            token
          )
          .then((res: TColor) => {
            // eslint-disable-next-line @typescript-eslint/strict-boolean-expressions
        ...
        imageAPI
          ...
          .then((res) => {
            colorApi
              .updateColor(
                {
                  ...restData.colorsSchema_id,
                  useBackgroundImg: res.file_name
                },
                token
              )
              .then((res: TRest) => {
      ...
      openNotification(t('you-not-change-background-image'), 'topRight')
    }
  }
  return (
    <>
